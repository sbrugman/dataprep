...
STATS = {"cleaned": 0, "null": 0, "overflow": 0, "bad_format": 0, "unknown": 0}
...
def fix_domain_name(domain_part: str) -> str:
    """
    Function to fix domain name with frequent typo
    """
    if domain_part.lower() not in DOMAINS:
        for i, curr_c in enumerate(domain_part):
            # two neighbor chars in reverse order
            if (domain_part[0:i] + domain_part[i + 1 :]).lower() in DOMAINS:
            ...
            for new_c in "abcdefghijklmnopqrstuvwxyz":
                if (
                    domain_part[0 : i + 1] + new_c + domain_part[i + 1 :]
                ).lower() in DOMAINS:
                    domain_part = (
                        domain_part[0 : i + 1] + new_c + domain_part[i + 1 :]
                    ).lower()
            ...
            if curr_c in NEARBYKEYS:
                for c_p in NEARBYKEYS[curr_c]:
                    if (
                        domain_part[0:i] + c_p + domain_part[i + 1 :]
                    ).lower() in DOMAINS:
                        domain_part = (
                            domain_part[0:i] + c_p + domain_part[i + 1 :]
                        ).lower()
...
def format_email(
    ...
    if valid_type == "null":
        return not_email(row, col, split, "null", errors)

    elif valid_type == "bad_format":
        return not_email(row, col, split, "bad_format", errors)

    elif valid_type == "overflow":
        return not_email(row, col, split, "overflow", errors)

    elif valid_type == "unknown":
        return not_email(row, col, split, "unknown", errors)

    elif valid_type != "valid":
        raise ValueError("invalid return type for instance check")
...
def validate_email(x: Union[str, pd.Series]) -> Union[bool, pd.Series]:
    ...
    if isinstance(x, pd.Series):
        return x.apply(check_email, False)
    else:
...
def report_email(nrows: int) -> None:
    ...
    print(
        f"""Result contains {ncorrect} ({pcorrect}%) values\
        in the correct format and {nnull} null values ({pnull}%)"""
    )
